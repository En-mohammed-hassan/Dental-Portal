...
import { formatAppointmentDate } from "@/utils/patient"
    ...
    <Dialog onOpenChange={onOpenChange} open={open}>
      <DialogContent className="max-h-[85vh] overflow-hidden flex flex-col sm:max-w-2xl lg:max-w-3xl">
        <DialogHeader>
          <DialogTitle className="text-lg sm:text-xl">
            {t("linkedReservations.title", {
              name: patient?.name ?? t("linkedReservations.titleFallback"),
            })}
          </DialogTitle>
            ...
            {hasReservations
              ? t(
                  reservations.length === 1
                    ? "linkedReservations.countDesc"
                    : "linkedReservations.countDescPlural",
                  { count: reservations.length }
                )
              : t("linkedReservations.emptyDesc")}
          </DialogDescription>
        </DialogHeader>

        {patient ? (
          <PatientBillingPanel
            totalChargedCents={patient.totalChargedCents ?? 0}
            totalPaidCents={patient.totalPaidCents ?? 0}
            ...
            {hasReservations ? (
                ...
                {reservations.map((reservation, index) => {
                  const statusConfig =
                    reservationStatusConfig[
                      reservation.status as keyof typeof reservationStatusConfig
                    ] ?? {
                  ...
                  const bookingConfig =
                    bookingTypeConfig[
                      reservation.bookingType as keyof typeof bookingTypeConfig
                    ] ?? {
                      label:
                        bookingTypeLabels[
                          reservation.bookingType as keyof typeof bookingTypeLabels
                      ...
                      <div className="flex flex-col gap-3 sm:gap-4">
                        ...
                        <div className="flex flex-wrap items-center justify-between gap-2">
                          ...
                          {reservation.completedAt && (
                            <span className="text-muted-foreground text-xs">
                              ...
                              {formatDate(reservation.completedAt, locale, {
                                hour: "2-digit",
                                minute: "2-digit",
                              })}
                            </span>
                        ...
                        <div className="grid grid-cols-1 gap-2 text-sm sm:grid-cols-2 lg:grid-cols-3">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-medium">
                              {formatAppointmentDate(reservation.appointmentDate, locale)}
                            </span>
                          </div>
                          <div className="flex items-center gap-2">
                            <UserCheck className="text-muted-foreground h-4 w-4 flex-shrink-0" />
                            <span className="text-muted-foreground">{t("linkedReservations.arrived")}:</span>
                            <span className={reservation.hasArrived ? "font-medium text-green-600 dark:text-green-400" : "font-medium text-slate-500"}>
                              {reservation.hasArrived ? t("linkedReservations.yes") : t("linkedReservations.no")}
                            </span>
                          </div>
                        </div>
                            ...
                            <button
                              ...
                              onClick={() =>
                                setPreviewXray({
                                  ...
                                  name: `${patient?.name ?? t("linkedReservations.titleFallback")} - ${formatAppointmentDate(reservation.appointmentDate)}`,
                                })
                              }
                              type="button"
                            >
                          ...
                          >
                            <div className="flex items-start gap-2">
                              ...
                              <div className="flex-1 min-w-0">
                                <p className="text-xs font-medium text-primary mb-1">{t("history.treatmentNote")}</p>
                        ...
                        <div className="pt-2 border-t">
                          <p className="text-muted-foreground text-xs">
                            {t("linkedReservations.created")}: {formatDate(reservation.createdAt, locale)}
                          </p>
                        </div>
                      </div>
                    </motion.div>
              ...
              >
                ...
                <p className="text-muted-foreground text-sm font-medium mb-1">
                  {t("linkedReservations.noReservations")}
                </p>
                <p className="text-muted-foreground text-xs">
                  {t("linkedReservations.noReservationsDesc")}
                </p>
              </motion.div>
            )}
          </AnimatePresence>
      ...
      >
        <DialogContent className="sm:max-w-2xl">
          <DialogHeader>
            <DialogTitle>{previewXray?.name ?? t("linkedReservations.xrayPreview")}</DialogTitle>
